...
// Test combinations
TEST(TestCore, Combinations)
{

  // Empty vector - no combinations
  {
    const Container_Type kEmptyCollection = Container_Type();
    Output_Container_Type combinations = SHA_Collections::Combinations<Container_Type, Const_Iterator_Type>
      (kEmptyCollection.begin(), kEmptyCollection.end());
  ...
  // String run
  {
    ...
    std::list<std::string> combinations =
      SHA_Collections::Combinations<std::string, std::string::const_iterator>(abcStr.begin(), abcStr.end());
    EXPECT_EQ(7, combinations.size());
